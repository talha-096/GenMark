...
class AnimationController {
  ...
  public render() {
    ...
    // Transparent clear so the page background shows through
    ...
    // White dots with 70% opacity so dark background bleeds through slightly
  ...
  private drawTrail(t1: number) {
    for (let i = 0; i < this.trailLength; i++) {
      ...
      this.ctx.fillStyle = 'rgba(150, 180, 255, 0.85)'
      this.ctx.lineWidth = sw
...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (displacementProgress < 0.3) {
      const localT = displacementProgress / 0.3
      screenX = controller.lerp(spiralPos.x, spiralPos.x + this.dx * 0.3, localT)
      screenY = controller.lerp(spiralPos.y, spiralPos.y + this.dy * 0.3, localT)
    } else if (displacementProgress < 0.7) {
    ...
    // Access private fields via bracket notation (same-file JS runtime access)
    const cameraZ = (controller as unknown as Record<string, number>)['cameraZ']
    const viewZoom = (controller as unknown as Record<string, number>)['viewZoom']
