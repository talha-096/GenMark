...
class BrandKit:
    ...
    def get_by_user(user_id):
        return list(db.db.brand_kits.find({"user_id": ObjectId(user_id)}))

    @staticmethod
    def get_by_project(project_id):
        return list(db.db.brand_kits.find({"project_id": ObjectId(project_id)}))

    @staticmethod
    def get_by_id(kit_id):
        return db.db.brand_kits.find_one({"_id": ObjectId(kit_id)})
    ...
    def update_brand_kit(kit_id, data):
        ...
        return db.db.brand_kits.update_one(
            {"_id": ObjectId(kit_id)},
            {"$set": data}
        )
    ...
    def delete_brand_kit(kit_id):
        return db.db.brand_kits.delete_one({"_id": ObjectId(kit_id)})
